...
10월 29일 해커톤 대비 음성파일로 감정일기 작성하여 내 매일매일의 상태를 점검하는 APP 프로그램의구현

## 페이지

1. 배너 페이지 - 음성녹음 버튼(큰거), 아래에 글씨로 ‘나의 녹음 확인’ 누르면 메인페이지로 이동하는 버튼, 그리고 ‘목소리로 나의 일기를 완성해보세요’ 소개 글 맨 위에 이렇게 3개 구성
2. 녹음 중일 때 페이지 - 목소리 녹음 시 녹음 버튼 켜지는 거 활성화, 한번 더 누를 시 로딩 중 페이지를 거쳐서 db에 적재 - 메인페이지에 표출
3. 로딩 중 페이지 - GptAI를 이용해 4개의 컬럼에맞는 데이터(날짜, 제목(20줄 이내), 요약 내용, 감정상태_점수 또는 기쁨/슬픔/무난)로 변환 후 PostgreDB로 적재 하는 과정 중에 이렇게 뜬다
4. 메인 페이지 - 게시판(개략 - 10개 당 1페이지), 일자 별 감정변화 그래프 있고, 홈으로 버튼 누르면 배너로 다시 돌아감
5. 게시판(상세) 페이지 - 수정, 삭제, 목록으로 버튼 있음, 글 제목 수정 불가
6. 게시판(상세 - 수정)페이지 or 상태변화 - 글 제목, 내용이 수정 가능하도록 변환됨

## 개발 순서

1. 배너 페이지 → 음성 녹음 버튼(실제 녹음 기능)과, 나의 일기 확인하기 버튼(게시판, 그래프가 있는 페이지로 이동) 아래에 음성 녹음으로 당신만의 감정일기를 작성해보세요! 라는 안내 문구 만들기
2. 음성 데이터 텍스트로 변환(STT)
3. 텍스트로 변환 된 데이터를 gpt api를 이용해 내용 요약 후 DB에 들어갈 수 있도록 [날짜, 제목(20글자 이내), 요약된 내용, 감정-정해진 템플릿 단어 또는 숫자로] 변환하기
4. 2의 변환된 데이터를 PostgreDB에 저장하기
5. 게시판 작성하기 → 날짜, 제목의 순으로 10개당 1항목으로 리스트업하기
6. 게시판 클릭 시 수정, 삭제, 목록 보기 버튼 있고 제목 날짜 내용 순으로 보여주기
7. 날짜 별 감정 코드 읽어서 그래프화 시키기 → 그래프는 게시판 하단에 위치

## 진행 상황

1. 데이터베이스 준비 (Docker로 PostgreSQL 실행) - 완료
2. 백엔드 구축 (STT- + 요약/감정 + CRUD) - 완료(HealthCheck)
    
    ![{BA58BE35-0A14-44B1-8393-DDF666F7246A}.png](attachment:3d416499-9eca-46ff-a39a-f5b1d9483afc:BA58BE35-0A14-44B1-8393-DDF666F7246A.png)
    
3. 프런트엔드(React + Vite) 구축 - 완료
4. 태스크별 실행·검증 (요구사항 매핑) - 완료, 환각 일어남


> **개인 생성형ai (2025.10)** 프로젝트  
...
| ETC | Nginx(정적 서빙 + API 프록시), dotenv, zod |
...
- OpenAI API Key  
  → [OpenAI Dashboard](https://platform.openai.com/api-keys)에서 발급
...
2. `server/.env.example` 을 참고해 백엔드 설정 가능:
   ```bash
